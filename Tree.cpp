...
public:
	...
	void OutCompWeight(string fileName)
	{
		...
		int sum_all_comp = 0; map<int, int> sum_all_comp_course;
		int sum_VU = 0;
		int sum_VU_2 = 0;
		...
		for (auto it : Global->dictory)
		{
			...
			for (auto st : it.second.Session)
			{
				for (auto et : it.second.Comp)
				{
					CW_course[(st - 1) / 2 + 1][et] += it.second.VU / float(it.second.Comp.size());
				}
				sum_all_comp_course[(st - 1) / 2 + 1] += it.second.VU;
